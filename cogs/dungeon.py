...
class Dungeon(commands.Cog):
    ...
    async def dungeon(self, ctx):
        ...
        helpers = Helpers()
        cur.execute("select plvl, pexp, strh, dex, intl from Player where discord_id = %s;", (ctx.message.author.id,))
        r = cur.fetchone()
        curLvl = r[0]
        curExp = r[1]
        curSTR = r[2]
        ...
        if playerExist:
            ...
            cur.execute("update Player set pexp = %s where discord_id = %s", (curExp, ctx.message.author.id))
            if helpers.checkIfPlayerLeveledUp(ctx.message.author.id, curLvl, curExp):
                embed = discord.Embed(color=discord.Color.dark_teal())
                embed.add_field(name="** **",value=f"**You have leveled up!**  \nLevel: {curLvl} (+{1})\nSTR: {curSTR} (+{1})\nINT: {curINT} (+{1})\nDEX: {curDEX} \
                    (+{1})" )
                cur.execute("update Player set plvl = %s, strh = %s, intl = %s, dex = %s where discord_id = %s;", (curLvl+1, curSTR+1, curINT+1, curDEX+1, ctx.message.author.id))
